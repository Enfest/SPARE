...
def unroll_multicontrolled_slice(self, slices, sliceid=0, edge_slice_copy=None, debug=False, works_on_two=False, use_controls=None):
    ...
    def unroll_a_gate(node, new_slices, flag=False, mk_new_slice=False, use_num_controls=None):        
        ...
        # print("new slices are ")
        # print(new_slices)
        return new_slices

    nodes_realized = []
    new_slices = []
    # print("is works on 2 ", works_on_two)
    for i, node in enumerate(slices.node_map):
        if node is not None and node.get_node_type().is_gateset():
            ...
            flag = works_on_two or edge_slice_copy.node_works_on_qutrit(sliceid, node)[0]
            # print(edge_slice_copy)
            # if debug:
            #     print("fetch for ", works_on_two, edge_slice_copy.node_works_on_qutrit(sliceid, node)[0])
            # else:
            #     print("this is deeper ", works_on_two, edge_slice_copy.node_works_on_qutrit(sliceid, node)[0])
...
def convert_gateslices(self):
    ...
    print(if_ancilla)
    print("The ancilla mapping status is ", ancilla_maps)
    ...
    # print("first slice collection is ", self.slice_collection[0])
    # print("node name maps ", [x._is_ancilla for x in self.slice_collection[0].node_map])
    # print("is ancilla details ", end="")
    # for x in self.slice_collection[0].node_map:
    #     if x is not None:
    #         print(x.is_ancilla(), end=", ")
    #     else:
    #         print("None", end=", ")
    # print()
    return
...
def unroll_multicontrolled_gates(self):
    ...
    # for r in self.slice_edge_collection.list_of_edges:
    #     print(r)
    ...
    self.get_all_matrix_mergable_gates(debug=False)
    self.prune_null_slices()           
    # print("Multiple controls unrolled here")
