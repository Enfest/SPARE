...
# from rewriter.src.rewrites import apply_rule_gate_merge


# def remove_high_control_gates(gateslice):
...
#         res = gateslice.remove_high_control_gates(i)
#         res = res or fullres
#     return gateslice, res

# def remove_high_phase_gates(gateslice):
#     slice_max, edge_max = gateslice.return_iterator()
#     for i in range(1, slice_max-1):
#         gateslice.remove_high_phase_gates(i)
#     return gateslice
...
def verify_gateslices(gateslices, if_verify, unitary1):
  ...
  print("Verify:   ", gateslices.dimension)
  if if_verify=="TRUE":
    print("ACTUALLY VERIFYING")
    print("dimension was ", gateslices.dimension, [x for x in gateslices.slice_collection[0].node_map if not x._is_ancilla])
    ...
    a, b = check_circuit_equality(unitary1, unitary2, dimension=gateslices.dimension,
...
def collapse_states(gateslice, if_verify=True, unitary=None):
    ...
    basic_pass(gateslice, "collapse_states", if_verify, unitary1)
    gateslice.slice_edge_collection.update_all_mappings()
    # print(gateslice.build_circuit()[0])
    for _ in range(2):
    ...
    # gateslice.remove_phases()
    gateslice.merge_greedily()
    # print("FULL COLLAPSE DONE CHECK NOW --- ")
    # c, _ = gateslice.build_circuit()
    # print(c)
    for _ in range(2):
        gateslice.get_all_matrix_mergable_gates(debug=False)
        # c, _ = gateslice.build_circuit()
        # print("COLLASE STATES PASS 2nd time")
        # print(c)
    verify_gateslices(gateslice, if_verify, unitary1)
    return gateslice
